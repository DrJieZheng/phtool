...
```bash
python -m phtool cutimage [输入文件列表] --out-dir [输出路径] \
    --cut-x [裁剪x坐标] --cut-y [裁剪y坐标] --cut-w [裁剪宽度] --cut-h [裁剪高度]
```
...
```bash
python -m phtool cutimage image*.fits --out-dir cutted \
    --cut-x 100 --cut-y 100 0.9 --cut-w 1000
```
...
phtool.cutimage(filelist, "cutted", 
                cut_x=100, cut_y=[100, 0.9], cut_w=1000)
```
...
```bash
python -m phtool biascombine [本底文件列表] \
    --bias [输出本底文件] --combine [合并方法]
```
...
```bash
python -m phtool biascombine bias1.fits bias2.fits bias3.fits \
   --bias master_bias.fits --combine median
```
...
```bash
python -m phtool flatcombine [平场文件列表] \
    --bias [本底文件] --flat [输出平场文件] \
    --combine [合并方法] --norm [归一化方法]
```
...
```bash
python -m phtool flatcombine flat1.fits flat2.fits flat3.fits \
    --bias master_bias.fits --flat master_flat.fits \
    --combine clip --norm mean
```
...
```bash
python -m phtool imcorr [科学图像列表] --outdir [输出目录] \
    --bias [本底文件] --flat [平场文件] \
    --radec [目标坐标] --keyradec [坐标关键字] \
    --sitename [观测站点] --sitecoord [站点坐标]
```
...
```bash
python -m phtool imcorr sci1.fits sci2.fits --outdir corrected  \
    --bias master_bias.fits --flat master_flat.fits \
    --radec "12:34:56.78,-12:34:56.78" \
...
```bash
python -m phtool offset [图像列表] --offsetfile [偏移文件] \
    --baseix [基准图像索引] --maxoffset [最大偏移]
```

**参数说明**：

- `图像列表`：需要计算偏移的图像
- `--offsetfile`：偏移文件，默认生成 `offset.txt`
- `--baseix`：基准图像索引，默认为0
...
```bash
python -m phtool offset corrected/*fits --offsetfile offset.txt \
    --baseix 0 --maxoffset 500
```
...
```bash
python -m phtool align [图像列表] --alignfile [对齐文件] --baseix [基准图像索引]
```

**参数说明**：

- `图像列表`：需要对齐的图像
- `--alignfile`：对齐文件，默认生成 `align.pkl`
- `--baseix`：基准图像索引，默认为0

**示例**：

```bash
python -m phtool align corrected/*fits --alignfile align.pkl --baseix 0
```
...
- `图像列表`：需要测光的图像
- `--apers`：测光孔径，默认为[-2.5]，负数表示相对于FWHM的倍数，正数表示具体孔径。可以是多个值。对于正孔径，对每张图都是同一个孔径进行测光；而负孔径，每张图都是根据自己的平均半高全宽来进行。

**示例**：

```bash
python -m phtool phot corrected/*fits --apers -2.5 -3.5 10 15 20
```
...
```bash
python -m phtool display [图像文件] \
    --show-x [x坐标范围] --show-y [y坐标范围] --show-n [显示数量]
```
...
**示例**：
```bash
python -m phtool display corrected/*fits \
    --show-x 0 1 --show-y 0.2 --show-n 10
```
...
```bash
python -m phtool xyget [图像列表] --xyfile [选星结果文件] \
    --baseix [基准图像索引] --pickbox [选星范围]
```
...
```bash
python -m phtool xyget corrected/*fits \
    --xyfile selected_stars.txt \
    --baseix 0 --pickbox 20
```
...
```bash
python -m phtool pick [图像列表] --pickfile [选星结果文件] \
    --xyfile [选星坐标文件] --alignfile [对齐文件] \
    --baseix [基准图像索引] --pickbox [选星范围]
```
...
- `--alignfile`：对齐文件，默认使用 `align.pkl`
- `--baseix`：基准图像索引，默认为0
- `--pickbox`：选星范围，默认为20

**示例**：
```bash
python -m phtool pick corrected/*fits --pickfile pick.pkl \
    --xyfile selected_stars.txt --alignfile align.pkl \
    --baseix 0 --pickbox 20
```
...
#### 终端调用模式
```bash
python -m phtool diffcali --pickfile [选星结果文件] --califile [差校结果文件] \
    --tgtidx [目标星索引] --refidx [参考星索引] --chkidx [检查星索引]
```
...
**示例**：
```bash
python -m phtool diffcali --pickfile pick.pkl --califile cali.pkl \
    --tgtidx 0 --refidx 1 2 --chkidx 3 4
```
